...
const Navbar: React.FC<NavbarProps> = ({ toggleTheme, currentTheme }) => (
  <motion.nav
    ...
    className="fixed left-0 right-0 top-4 z-50 px-4 sm:px-7"
  >
    <div className="mx-auto flex max-w-[1160px] items-center justify-between rounded-full border border-white/10 bg-[#12100f]/85 px-5 py-3 backdrop-blur-xl md:px-7">
      <a
        href="#"
        className="font-heading text-sm font-bold tracking-[-0.06em] text-[#f7f2ec]"
      >
        Gopal<span className="text-[#ef7840]">.</span>
      </a>
      <div className="hidden items-center gap-6 md:flex">
        <NavLink href="#about">About</NavLink>
        <NavLink href="#experience">Experience</NavLink>
        <NavLink href="#products">Work</NavLink>
      ...
      <div className="flex items-center gap-3">
        ...
        <a
          href="#contact"
          className="rounded-full bg-[#f7f2ec] px-4 py-2 text-[10px] font-bold text-[#171413] transition-transform hover:scale-[1.03]"
        >
          Let&apos;s talk
        </a>
      </div>
